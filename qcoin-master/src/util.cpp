...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "qcoin" is a composite category enabling all Qcoin-related debug output
            if(ptrCategory->count(string("qcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\qcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\qcoinCore
    // Mac: ~/Library/Application Support/qcoinCore
    // Unix: ~/.qcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "qcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/qcoinCore";
#else
    // Unix
    return pathRet / ".qcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty qcoin.conf if it does not excist
